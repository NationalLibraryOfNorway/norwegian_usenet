...
referenced id) pair in `message_references`. A user is an email address, which
`messages.email_id` names. Message ids and bodies appear only as hashes, so the
archive's own file can be shared. No free text is stored at all: subjects and the
Newsgroups header carried plain text addresses and message ids of their own
(cancel messages name their target id in the subject, mis-addressed posts put an
address in the Newsgroups list), which would have handed back the plain text the
hashing is there to withhold.
...
Row ids are handed out per database, so an id only means anything together with
the archive it was read from. `connect_archives` opens
