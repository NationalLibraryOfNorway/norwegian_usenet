...
def select_newsgroups(graph: nx.Graph, selection: list[str]) -> nx.Graph:
    ...
    would otherwise quietly leave a newsgroup out of the picture.
    """
    missing = sorted(set(selection) - set(graph.nodes))
    if missing:
        raise ValueError(f"Newsgroups not in the overlap table: {', '.join(missing)}")

    selected = nx.Graph(graph.subgraph(selection))
    logger.info(
