...
- [10_count_unescaped_ia_from_lines.py](10_count_unescaped_ia_from_lines.py) reads every IA mbox file listed in `data/input/internet_archive/encodings.json` and writes one row per line beginning with `From ` that the envelope rule rejected, with the file it is in, the line it is on and whether a message header block follows it, to `data/output/01_extract_and_parse_usenet_data/ia_unescaped_from_lines.csv`. The lines themselves are message bodies, and are not written out.
...
Every IA envelope line carries the Google Groups id the archive was scraped with, a `From ` and a signed integer, so `usenet_no.mbox_utils.StrictMbox` accepts only a line of that form and reads the rest as body text. That leaves 3497 `From ` lines across the archive, spread over 242 of the 331 source files. [10_count_unescaped_ia_from_lines.py](10_count_unescaped_ia_from_lines.py) counts them and reads the lines under each one: no `X-Google-Language` header follows any of them, so none of them begins a message. The file and line of every one is in [ia_unescaped_from_lines.csv](../../data/output/01_extract_and_parse_usenet_data/ia_unescaped_from_lines.csv).
