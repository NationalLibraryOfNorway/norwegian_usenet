...
- [01_build_database.py](01_build_database.py) reads the .mbox files of both archives and writes the shared and private SQLite databases in one pass. The shared database, `data/output/02_build_database/usenet.db`, holds one row per message with names, emails and message ids as hashes only, so it can be shared. The private one, `data/output/02_build_database/usenet_private.db`, maps those hashes back to their plain text. The two are built as a pair, since they assign ids together.
...
The scripts below print a fingerprint of the rows themselves rather than of the .db files,
which are not byte-reproducible between builds. Run one on each machine and compare the
output. They read the databases read-only, and use nothing outside the standard library,
so they can be copied to a machine that has neither this repository nor its dependencies.

- [02_fingerprint_database.py](02_fingerprint_database.py) hashes every table of both databases, row ids included. Start here: if every line matches, the two databases are interchangeable.
