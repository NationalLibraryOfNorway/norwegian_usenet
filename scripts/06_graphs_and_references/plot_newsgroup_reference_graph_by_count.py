"""Draw the reference edge lists as a directed graph of newsgroups, counting the references.

The alternative to 04_plot_newsgroup_reference_graph.py, which weighs an edge
against the referring newsgroup instead: here an edge is drawn when enough
references run along it, whatever share of its newsgroup's references that is,
so the busiest newsgroups keep the most edges.

Reads an edge list written by 06_graphs_and_references and keeps the edges
carrying at least a given number of references. They are drawn as arrows from
the referring newsgroup to the referenced one, with the width following the
number of references. The two directions between a pair are drawn as two curves, so both
...
from usenet_no.newsgroup_graph import (
    build_reference_graph_by_count,
    load_message_counts,
)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=("Draw newsgroups joined by the number of references between them"),
    ...
    graph = build_reference_graph_by_count(
    ...
    plot_reference_graph(
        ...
        subtitle=(
            ...
            f"{args.min_references:,} references"
        ),
