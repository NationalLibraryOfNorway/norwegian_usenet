...
- [02_embed_messages.py](02_embed_messages.py) makes text embeddings for each message in a selection of newsgroups from both archives, with a sentence-transformers model. Writes to `data/output/05_make_embeddings/<model>/`.
...
- [03_umap_reduce_embeddings.py](03_umap_reduce_embeddings.py) reduces the embeddings to 2 dimensions with UMAP, and caches the result in `data/output/05_make_embeddings/umap_embeddings/<model>/`.
