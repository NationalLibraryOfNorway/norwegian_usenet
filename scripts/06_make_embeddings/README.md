...
- [02_embed_messages.py](02_embed_messages.py) makes text embeddings for each message in a selection of newsgroups from both archives, with a sentence-transformers model. Writes to `data/output/06_make_embeddings/<model>/`.
- [02_make_jina_embeddings.py](02_make_jina_embeddings.py) does the same with a Jina embedding model. It is an alternative to `02_embed_messages.py`, hence the same number prefix — run one or the other.
- [03_umap_reduce_embeddings.py](03_umap_reduce_embeddings.py) reduces the embeddings to 2 dimensions with UMAP, and caches the result in `data/output/06_make_embeddings/umap_embeddings/<model>/`.
