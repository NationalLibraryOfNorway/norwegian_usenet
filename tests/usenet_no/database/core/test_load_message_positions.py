...
def test_positions_start_at_zero_in_each_newsgroup(mbox_data, database, load_archives):
...
def test_keeps_only_the_messages_inside_the_span(mbox_data, database, load_archives):
    """no.dated.ids.mbox holds one message in 1996 and one in 2005."""
    connection = load_archives(
        database, [(mbox_data / "ia/no.dated.ids.mbox", IA_ARCHIVE)]
    )
...
def test_drops_messages_whose_date_did_not_parse(mbox_data, database, load_archives):
    """The undated message sits between the two dated ones, at position 1."""
    connection = load_archives(
        database, [(mbox_data / "nb/no.mixed.dates.mbox", NB_ARCHIVE)]
    )
...
def test_keeps_undated_messages_without_a_span(mbox_data, database, load_archives):
    connection = load_archives(
        database, [(mbox_data / "nb/no.mixed.dates.mbox", NB_ARCHIVE)]
    )
...
def test_reads_one_archive_at_a_time(mbox_data, database, load_archives):
...
def test_a_newsgroup_with_no_message_in_the_span_is_left_out(
