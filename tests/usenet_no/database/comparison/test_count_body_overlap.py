...
def test_counts_a_body_both_archives_have_as_shared(mbox_data, load_archives):
...
def test_a_crossposted_body_counts_once(mbox_data, load_archives):
    connection = load_archives(
        [
...
def test_messages_with_an_empty_body_are_left_out(mbox_data, load_archives):
    connection = load_archives([(mbox_data / "nb/no.count.third.mbox", NB_ARCHIVE)])
